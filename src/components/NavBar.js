...
function NavBar(props) {
    ...
    const renderBackMobile = () => {
        let id
        if (props.page <= 0) id = "back-invisible-mobile"
        else { id = "back-visible-mobile" }
        return <a href="#home" id={id} className={back} onClick={() => hamClick()}><svg className="nav-icon-mobile browser-window__link-back" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path id="back-path" d="M10.8333 6.5235V16.6668H9.16664V6.5235L4.69664 10.9935L3.51831 9.81516L9.99998 3.3335L16.4816 9.81516L15.3033 10.9935L10.8333 6.5235Z" fill="black" />
        </svg><p className="back-text-mobile">Back to top</p>
        </a>
    }
    ...
    const colorHandler = () => {
        ...
        if (props.page > 0 && props.page <= projects.length) return <div style={{color: "#FF517B"}} id={text}>Work</div>
        else if (props.page == (projects.length + 1)) return <div style={{color: "#A35AFF"}} id={text}>About</div>
        else if (props.page == (projects.length + 2)) return <div style={{color: "#4478FF"}} id={text}>Contact</div>
    }

    const [name, setName] = React.useState(null)
    const [nav, setNav] = React.useState(null)
    const [text, setText] = React.useState("text-nav")
    const [ham, setHam] = React.useState("ham-menu-initial")
    const [back, setBack] = React.useState("back-hide")
    ...
    const hamClick = () => {
        if (name === "active") {
            ...
            setName("active") 
            setNav("open")
            setText("text-nav-hidden")
            setHam("ham-menu-active")
            setBack(null)
        }
    }
    ...
    return (
        <div id={nav} className={"nav"}>
            {renderBack()}
                <div className={"hamburger hamburger--minimal " + name} onClick={() => hamClick()} type="button">
                    <div className="inner">
                ...
                {colorHandler()}
                <div className={ham}>
                    <a className="menu-options" href={"#" + projects[0].url} onClick={() => hamClick()}>Work</a>
                    <a className="menu-options" href="#about" onClick={() => hamClick()}>About</a>
                    <a className="menu-options" href="#contact" onClick={() => hamClick()}>Contact</a>
                    {renderBackMobile()}
                </div>
                <div className="menu">
